...
console.log(canWeDeliver(3893029))
...
const canWeDeliverTwo = zipCode => {
    let result = ""
    deliveryAreaZipCodes.forEach(z => {
        if(z === zipCode){
            result = `You're in our delivery zone!`
        }else{
            result = `Sorry, we can't deliver to that address`
        }
    })
    return result
}
...
firstDeal.title = firstDeal.title.replace('15','10')
console.log(firstDeal.title)
...
secondDeal.desc = secondDeal.desc.replace('March', 'April')
console.log(secondDeal.desc)